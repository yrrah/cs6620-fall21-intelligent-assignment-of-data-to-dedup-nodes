          ...
          "spec": {
              "containers": [{
                      ...
                      "env": [{
                              ...
                              "value": "'"$(oc get pods --selector app=cs6620-fall21-dedup-nodes-back-end-ex0 --field-selector status.phase=Running -o jsonpath='{range .items[:-1]}{.status.podIP}{","}{end}{.items[-1].status.podIP}')"'"
                      }],
                      "volumeMounts": [{
                          "mountPath": "/var/input",
                          "name": "input-traces"
