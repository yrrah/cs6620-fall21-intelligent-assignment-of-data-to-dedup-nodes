...
kill_old_pods () {
  oc delete jobs cs6620-fall21-dedup-nodes-back-end-ex1
  oc delete pods cs6620-fall21-dedup-nodes-front-end-ex1
}
...
create_back_end () {
  echo '{
    ...
    "kind": "Job",
    "metadata": {
        "name": "cs6620-fall21-dedup-nodes-back-end-ex1",
        "namespace": "cs6620-fall21-intelligentassignment-dedupnodes"
    },
    "spec": {
        "parallelism": '"$num_pods"',
        "selector": {
            "app": "cs6620-fall21-dedup-nodes-back-end-ex1"
        },
        "template": {
            "metadata": {
                "labels": {
...
create_front_end () {
  oc run cs6620-fall21-dedup-nodes-front-end-ex1 --overrides='
  {
          "spec": {
              "containers": [{
                      "name": "cs6620-fall21-dedup-nodes-front-end-ex1",
                      ...
                      "env": [{
                              ...
                              "value": "'"$(oc get pods --selector app=cs6620-fall21-dedup-nodes-back-end-ex1 --field-selector status.phase=Running -o jsonpath='{range .items[:-1]}{.status.podIP}{","}{end}{.items[-1].status.podIP}')"'"
                      }],
                      "volumeMounts": [{
                          "mountPath": "/var/input",
                          "name": "input-traces"
...
do
  ...
  while [[ $(oc get pods --field-selector metadata.name=cs6620-fall21-dedup-nodes-front-end-ex1) != *Completed* ]]
  do
