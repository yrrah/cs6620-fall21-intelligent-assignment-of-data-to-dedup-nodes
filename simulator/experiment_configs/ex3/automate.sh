...
kill_old_pods () {
  oc delete jobs cs6620-fall21-dedup-nodes-back-end-ex3
  oc delete pods cs6620-fall21-dedup-nodes-front-end-ex3
}
...
create_back_end () {
  echo '{
    ...
    "kind": "Job",
    "metadata": {
        "name": "cs6620-fall21-dedup-nodes-back-end-ex3",
        "namespace": "cs6620-fall21-intelligentassignment-dedupnodes"
    },
    "spec": {
        "parallelism": '"$num_pods"',
        "selector": {
            "app": "cs6620-fall21-dedup-nodes-back-end-ex3"
        },
        "template": {
            "metadata": {
                "labels": {
...
create_front_end () {
  oc run cs6620-fall21-dedup-nodes-front-end-ex3 --overrides='
  {
          "spec": {
              "containers": [{
                      "name": "cs6620-fall21-dedup-nodes-front-end-ex3",
                      ...
                      "env": [{
                              ...
                              "value": "'"$(oc get pods --selector app=cs6620-fall21-dedup-nodes-back-end-ex3 --field-selector status.phase=Running -o jsonpath='{range .items[:-1]}{.status.podIP}{","}{end}{.items[-1].status.podIP}')"'"
                      }],
                      "volumeMounts": [{
                          "mountPath": "/var/input",
                          "name": "input-traces"
...
for i in {1..31}; do read skip_these_lines; done;
while IFS=$'\t' read -r run_num dedup_domains num_pods region_algo region_size min_region max_region main_d second_d bitmask_size assign_algo q_learning epsilon q_penalty dataset results_file
do
  kill_old_pods
  create_back_end
  sleep 5
  ...
  while [[ $(oc get pods --field-selector metadata.name=cs6620-fall21-dedup-nodes-front-end-ex3) != *Completed* ]]
  do
